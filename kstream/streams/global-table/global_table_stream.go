...
import (
	...
	sarama2 "github.com/tryfix/kstream/consumer/sarama_dept"
...
type GlobalTableStreamConfig struct {
	ConsumerBuilder sarama2.PartitionConsumerBuilder
	...
	for _, topic := range info {
		...
		for i := int32(len(topic.Partitions)) - 1; i >= 0; i-- {
			partitionConsumer, err := config.ConsumerBuilder.Build(
				sarama2.BuilderWithId(fmt.Sprintf(`global_table_consumer_%s_%d`, topic.Name, i)),
				sarama2.BuilderWithLogger(config.Logger.NewLog(log.Prefixed(fmt.Sprintf(`global-table.%s-%d`, topic.Name, i)))),
			)
			if err != nil {
