...
//type NdList []topology.NodeBuilder
//
//func (s NdList) Len() int      { return len(s) }
//func (s NdList) Swap(i, j int) { s[i], s[j] = s[j], s[i] }
//
//type NdListSorted struct { NdList }
//
//func (s NdListSorted) Less(i, j int) bool { return s.NdList[i].Id().Id() < s.NdList[j].Id().Id() }

//func (b *kSubTopologyBuilder) ExtractSubTopology(node topology.NodeBuilder) topology.SubTopologyBuilder {
//	sbTp := new(kSubTopologyBuilder)
//	sbTp.kind = b.kind
//
//	var edges []topology.Edge
//	nodes := map[topology.NodeId]topology.NodeBuilder{}
//
//	fromedges := make([]topology.Edge, len(b.edges))
//	copy(fromedges, b.edges)
//
//	printnn := func(edgs []topology.Edge) {
//		for _, edge := range edgs{
//			fmt.Printf("%s - %s \n", edge.Parent().Id(), edge.Node().Id())
//		}
//	}
//
//
//	var find func(edgs []topology.Edge, linkedNodes []topology.NodeId)
//
//	find = func(edgs []topology.Edge, linked []topology.NodeId) {
//
//
//		for _, id := range linked{
//
//			//edgsCopy := make([]topology.Edge, len(edgs))
//			//copy(edgsCopy, edgs)
//			var edgsCopy []topology.Edge
//
//
//			var linkedNodes []topology.NodeId
//			for _, edge := range edgs{
//				if edge.Parent().Id() == id{
//					edges = append(edges, edge)
//
//					nodes[edge.Parent().Id()] = edge.Parent()
//					nodes[edge.Node().Id()] = edge.Node()
//
//					//nodes = append(nodes, edge.Node())
//					linkedNodes = append(linkedNodes, edge.Node().Id())
//					//edgsCopy = append(edgsCopy[:i], edgsCopy[i+1:]...)
//					continue
//					//find(edgs)
//					//continue
//				}
...
//				if edge.Node().Id() == id{
//					edges = append(edges, edge)
//					//nodes = append(nodes, edge.Parent())
//
//					nodes[edge.Node().Id()] = edge.Node()
//					nodes[edge.Parent().Id()] = edge.Parent()
//					linkedNodes = append(linkedNodes, edge.Parent().Id())
//					//edgsCopy = append(edgsCopy[:i], edgsCopy[i+1:]...)
//					continue
//				}
//
//				edgsCopy = append(edgsCopy, edge)
//
//			}
//
//			find(edgsCopy, linkedNodes)
//
//		}
//
//	}
...
//
//	printnn(fromedges)
//	find(fromedges, []topology.NodeId{node.Id()})
//	println(`after`)
//	printnn(fromedges)
//
//	println(`result`)
...
//
//	// sort nodes
//	var sorted NdListSorted
//	for _, nd := range nodes {
//		//b.RemoveNode(nd)
//		sorted.NdList = append(sorted.NdList, nd)
//	}
//	sort.Sort(sorted)
//
//	for _, nd := range sorted.NdList {
//		b.RemoveNode(nd)
//		sbTp.AddNode(nd)
//	}
//
//	for _, edge := range edges {
//		sbTp.AddEdge(edge.Parent(), edge.Node())
//	}
//
//	return sbTp
//}
